...
GHomepage = {
	...
	BuildMainCapsuleItem: function( rgItem, strFeatureContext, nDepth )
	{
		...
		if( rgRecommendationReasons.recommended_by_friend )
		{
			...
			var $ReasonMain = $J('<div/>').addClass('main').addClass('friend').html( "<strong>Aanbevolen<\/strong> door je vriend, <span>%1$s<\/span>".replace("%1$s", friend.name ) );
		...
		else if( rgRecommendationReasons.recent_release_by_creator && rgRecommendationReasons.recent_release_by_creator.creator_info )
		{
			...
			var $ReasonLocal = creator.link.indexOf( 'developer/' ) >= 0 ? "<strong>Ontwikkeld<\/strong> door<br><span>%1$s<\/span>" : "<strong>Uitgegeven<\/strong> door<br><span>%1$s<\/span>";
		...
		else if( rgRecommendationReasons.recommended_by_curator && rgRecommendationReasons.recommended_by_curator.rgCurators )
		{
			...
			var $ReasonMain = $J('<div/>').addClass('main').addClass('curator').html( "<strong>Aanbevolen<\/strong> door<br><span>%1$s<\/span>".replace("%1$s", V_EscapeHTML( curator.name ) ) );
		...
		else if( rgRecommendationReasons.recommended && GDynamicStore.s_rgRecommendedTags )
		{
			...
			if( rgMatchedTags.length == 0 )
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>Aanbevolen<\/strong> omdat je soortgelijke spellen speelt" );
			else
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>Aanbevolen<\/strong> omdat je spellen hebt gespeeld die zijn getagd met" );
		...
		if( bShowAdditionalReasons )
		{
			...
			for( var key in rgRecommendationReasons )
			{
				...
				if( key == 'top_seller' )
					strReason = "Bestverkocht";
				else if( key == 'new_release' )
					strReason = "Nieuwe uitgave";
				else if( key == 'recommended')
					strReason = "Vergelijkbaar met spellen die jij speelt";
				else
					continue;
				...
				$el.on('hover focus', function(){
					...
					else
					{
						...
						if ( rgData['review_summary'] )
						{
							...
							$elReviewData.append( $J('<div>', {'class': 'title'}).text( rgData.appids ? 'Gebruikersrecensies voor items in deze bundel:' : 'Algehele gebruikersrecensies:') );
						...
						if( rgData.tags )
						{
							...
							{
								var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/nl/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgData.tags[i] ) ), 'tab_preview' );
						...
						for( var i=0; i < 4; i++ )
						{
							...
							{
								var $elScreenshot = $J('<div>', {'class': 'screenshot empty'}).append("<div>").text("Deze screenshot bevat mogelijk inhoud voor volwassenen.");
								$elInfoDiv.append($elScreenshot);
							}
						}
	...
	RenderRecommendedByDeepDiveCarousel: function()
	{
		...
		focusedAppTitle = 'Omdat je <strong>%s</strong> hebt gespeeld'.replace( "%s", focusedAppTitle);
	...
	RenderCommunityRecommendations: function()
	{
		...
		{
			elemPrefLastUpdated.text( 'Je gebruikt voorkeuren die zijn opgeslagen op %1$s.'.replace( "%1$s", GHomepage.strCommunityRecommendationsPrefLastSaved ) );
		}
			...
			function( oItem, strFeature, rgOptions, nDepth )
			{
				...
				if ( oItem.announcementid.length != 0 )
				{
					...
					var $AnnouncementLink = $J ( '<div/>', {
						...
						'text': 'Details van de update weergeven',
	...
	FillPagedCapsuleCarousel: function( rgCapsules, $elTarget, fnCapsule, strNavContext, nCapsules )
	{
		...
		for( var j=0; j<rgCapsules.length; j+=nCapsules )
		{
			...
			for( var k=0; k < nCapsules && k + j < rgCapsules.length; k++ )
			{
				...
				{
					const panelString = '{"onOptionsActionDescription":"Alles weergeven","onOptionsButton":"window.location=\'%1$s\'"}';
	...
	BuildCommunityRecommendationCap: function( strFeatureContext, oItem )
	{
		...
		if ( numReviews > 1 )
		{
			...
			let strNumReviews = '%1$s van %2$s recensies'.replace( "%1$s", 1 ).replace( "%2$s", numReviews );
			...
			arrowLeft.click( function() {
				...
				var strNumReviews = '%1$s van %2$s recensies'.replace( "%1$s", curIdx + 1 ).replace( "%2$s", numReviews );
		...
		for( var i = 0; i < rgReviews.length; ++i)
		{
			...
			var $ViewMore = $J( '<div class="view_more"><a href="' + review.review_link + '" target="_blank">Volledige recensie lezen</a></div>' );
			...
			if ( reviewer )
			{
				...
				{
					var strHours = '%s uur gespeeld op moment van beoordeling'.replace( "%s", v_numberformat( review.playtime_at_review / 60, 1 ) );
					$AuthorDetails.append( $J( '<div>', { class: "hours ellipsis", text: strHours } ) );
				}
				if ( review.votes_up > 0 )
				{
					var voteUpText = '1 persoon vond deze recensie nuttig';
					if ( review.votes_up > 1 )
					{
						voteUpText = '%2$s mensen vonden deze recensie nuttig'.replace( "%2$s", v_numberformat( review.votes_up ) );
					}
	...
	RenderRecommendedTags: function()
	{
		...
		for ( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length; i++ )
		{
			...
			{
				var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/nl/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgTag.name ) ), 'gutter' );
...
{
	var $Btn = $J('<div/>', {'class': 'home_btn home_customize_btn' } ).text( 'Aanpassen' );
...
CHomeSettings.prototype.DisplayPopup = function( $Btn )
{
	...
	this.m_$Popup.append( $J('<div/>', {'class': 'home_viewsettings_instructions' } ).text( 'Kies de soorten producten die je in deze sectie wilt zien' ) );

	if ( this.m_ApplicableSettings.popular_new_releases )
		this.m_$Popup.append( this.RenderCheckbox( 'popular_new_releases', 'Populaire nieuwe uitgaven' ) );
	if ( this.m_ApplicableSettings.top_sellers )
		this.m_$Popup.append( this.RenderCheckbox( 'top_sellers', 'Bestverkocht' ) );
	if ( this.m_ApplicableSettings.early_access )
		this.m_$Popup.append( this.RenderCheckbox( 'early_access', 'Producten met vroegtijdige toegang' ) );
	if ( this.m_ApplicableSettings.games_already_in_library )
		this.m_$Popup.append( this.RenderCheckbox( 'games_already_in_library', 'Spellen reeds in je account' ) );
	if ( this.m_ApplicableSettings.games_not_in_library )
		this.m_$Popup.append( this.RenderCheckbox( 'games_not_in_library', 'Spellen niet in je account' ) );
	if ( this.m_ApplicableSettings.recommended_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_for_you', 'Aanbevolen voor jou' ) );
	if ( this.m_ApplicableSettings.prepurchase )
		this.m_$Popup.append( this.RenderCheckbox( 'prepurchase', 'Voorverkoop' ) );
	if ( this.m_ApplicableSettings.games )
		this.m_$Popup.append( this.RenderCheckbox( 'games', 'Spellen' ) );
	...
	if ( this.m_ApplicableSettings.dlc_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc_for_you', 'DLC voor je spellen en software' ) );
	if ( this.m_ApplicableSettings.recently_viewed )
		this.m_$Popup.append( this.RenderCheckbox( 'recently_viewed', 'Producten die je recent hebt bekeken' ) );
	if ( this.m_ApplicableSettings.new_on_steam )
		this.m_$Popup.append( this.RenderCheckbox( 'new_on_steam', 'Nieuw op Steam' ) );
	if ( this.m_ApplicableSettings.dlc )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc', 'Downloadbare content' ) );
	if ( this.m_ApplicableSettings.video )
		this.m_$Popup.append( this.RenderCheckbox( 'video', 'Video\'s' ) );
	if ( this.m_ApplicableSettings.localized )
		this.m_$Popup.append( this.RenderCheckbox( 'localized', 'Spellen in mijn taal' ) );
	...
	if ( this.m_ApplicableSettings.recommended_by_curators )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_by_curators', 'Aanbevolen door onze Steam-curators' ) );

	if ( this.m_ApplicableSettings.only_current_platform )
	{
		// this one is a little magic
		if ( GDynamicStore.s_bUserOnLinux )
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Beschikbaar voor Linux/SteamOS' ) );
		else if ( GDynamicStore.s_bUserOnMacOS )
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Beschikbaar voor Mac' ) );
	}
...
CHomeSettings.prototype.OnCheckboxToggle = function( strSettingName, $Checkbox )
{
	...
	$J.post( 'https://store.steampowered.com/dynamicstore/updatehomeviewsettings', {
		...
		ShowAlertDialog( 'Aanpassen', 'Er is een fout opgetreden bij het opslaan van je voorkeuren. Probeer het later nog eens.' );
	} );
};
...
GSteamCurators = {
    ...
    BuildResponsiveHomePageGiantCap: function( strFeatureContext, oItem )
	{
        ...
        var $Status = $J('<div/>', {'class': 'recommendation_status recommended'}).text( "Aanbevolen" );
